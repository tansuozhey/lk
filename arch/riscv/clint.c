...
// select the timer enable bit
#if ARCH_RISCV_MACHINE_MODE
#define IE_TIE_BIT RISCV_IE_MTIE
#else
#define IE_TIE_BIT RISCV_IE_STIE
#endif

static inline uint64_t get_tick(void) {
    //return *REG64(CLINT_MTIME);
    return riscv_csr_read("time");
}
...
#else
    return get_tick() / (ARCH_RISCV_MTIME_RATE / 1000000u);
#endif
}

lk_time_t current_time(void) {
    return get_tick() / (ARCH_RISCV_MTIME_RATE / 1000u);
}
...
status_t platform_set_oneshot_timer (platform_timer_callback callback, void *arg, lk_time_t interval) {
    ...
    // disable timer
    riscv_csr_clear(RISCV_REG(ie), IE_TIE_BIT);
    ...
    // enable the timer
    riscv_csr_set(RISCV_REG(ie), IE_TIE_BIT);

    sbi_set_timer(get_tick() + ticks);
